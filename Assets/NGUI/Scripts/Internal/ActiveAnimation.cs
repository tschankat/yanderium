...
public class ActiveAnimation : MonoBehaviour
{
	...
	Animation mAnim;
	AnimationOrTween.Direction mLastDirection = AnimationOrTween.Direction.Toggle;
	AnimationOrTween.Direction mDisableDirection = AnimationOrTween.Direction.Toggle;
	bool mNotify = false;
	...
	public void Finish ()
	{
		if (mAnim != null)
		{
			foreach (AnimationState state in mAnim)
			{
				if (mLastDirection == AnimationOrTween.Direction.Forward) state.time = state.length;
				else if (mLastDirection == AnimationOrTween.Direction.Reverse) state.time = 0f;
			}
			mAnim.Sample();
		}
	...
	public void Reset ()
	{
		if (mAnim != null)
		{
			foreach (AnimationState state in mAnim)
			{
				if (mLastDirection == AnimationOrTween.Direction.Reverse) state.time = state.length;
				else if (mLastDirection == AnimationOrTween.Direction.Forward) state.time = 0f;
			}
		}
		else if (mAnimator != null)
		{
	...
	void Update ()
	{
		...
		if (mAnimator != null)
		{
			mAnimator.Update((mLastDirection == AnimationOrTween.Direction.Reverse) ? -delta : delta);
			if (isPlaying) return;
	...
	void Play (string clipName, AnimationOrTween.Direction playDirection)
	{
		...
		if (mAnim != null)
		{
			...
			foreach (AnimationState state in mAnim)
			{
				if (string.IsNullOrEmpty(clipName) || state.name == clipName)
				{
					...
					if (playDirection == AnimationOrTween.Direction.Reverse && state.time == 0f) state.time = state.length;
					else if (playDirection == AnimationOrTween.Direction.Forward && state.time == state.length) state.time = 0f;
				}
			}
	...
	static public ActiveAnimation Play (Animation anim, string clipName, AnimationOrTween.Direction playDirection,
		EnableCondition enableBeforePlay, DisableCondition disableCondition)
	{
		...
		aa.mAnim = anim;
		aa.mDisableDirection = (AnimationOrTween.Direction)(int)disableCondition;
		aa.onFinished.Clear();
	...
	static public ActiveAnimation Play (Animation anim, AnimationOrTween.Direction playDirection)
	{
	...
	static public ActiveAnimation Play (Animator anim, string clipName, AnimationOrTween.Direction playDirection,
		EnableCondition enableBeforePlay, DisableCondition disableCondition)
	{
