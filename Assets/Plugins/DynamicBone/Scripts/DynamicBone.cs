...
public class DynamicBone : MonoBehaviour
{
    ...
    void Start()
    {
		if (m_DistantDisable)
		{
			...
			GameObject YandereChan = GameObject.Find("YandereChan");

			if (YandereChan == null)
			{
				m_ReferenceObject = transform;
			}
			else
			{
				m_ReferenceObject = YandereChan.transform;
			}
    ...
    {
		/*
		if (m_DistantDisable == false || m_DistantDisable == true && Vector3.Distance (transform.position, m_ReferenceObject.position) < m_DistanceToObject)
		{
    ...
    {
    	//Debug.Log("My name is " + this.gameObject.name + ". My root is named " + this.transform.root.gameObject.name);
    ...
    void InitTransforms()
    {
		//Debug.Log("The bone named " + name + " is currently updating. I belong to " + transform.root.name + "!");
