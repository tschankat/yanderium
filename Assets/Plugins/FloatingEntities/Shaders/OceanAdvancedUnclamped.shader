...
Shader "Nature/OceanAdvancedUnclamped"
{
	Properties
	{
	...
	inline void ComputeScreenAndGrabPassPos (float4 pos, out float4 screenPos, out float4 grabPassPos) 
	{
		...
		float4 object_position = mul(unity_ObjectToWorld, float4(0, 0, 0, 1));
		screenPos = ComputeScreenPos(object_position);
	...
	v2f vert(appdata v)
	{
		...
		/* Added object position to the world position*/
		float4 object_position = mul(unity_ObjectToWorld, float4(0, 0, 0, 1));
		world_position.y += object_position.y;
