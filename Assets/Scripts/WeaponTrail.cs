...
{
	[SerializeField] bool _emit = true;
	public bool Emit { set { _emit = value; } }

	[SerializeField] float _emitTime = 0.00f;
	[SerializeField] Material _material;
	[SerializeField] float _lifeTime = 1.00f;
	[SerializeField] Color[] _colors;
	[SerializeField] float[] _sizes;

	[SerializeField] float _minVertexDistance = 0.10f;
	[SerializeField] float _maxVertexDistance = 10.00f;

	[SerializeField] float _maxAngle = 3.00f;

	[SerializeField] bool _autoDestruct = false;
		...
		if (_emit)
		{
			if (theDistance > _minVertexDistance)
			{
				...
				else
				{
					...
					if ((Vector3.Angle(l1, l2) > _maxAngle) || (theDistance > _maxVertexDistance))
					{
						make = true;
					}
				}
		...
		if (pointsToUse.Count > 1)
		{
			...
			for (int n = 0; n < pointsToUse.Count; ++n)
			{
				...
				if ((_colors != null) && (_colors.Length > 0))
				{
					...
					float max = Mathf.Clamp(Mathf.Ceil(colorTime), 1.0f, _colors.Length - 1.0f);
					float lerp = Mathf.InverseLerp(min, max, colorTime);

					if (min >= _colors.Length)
					{
						min = _colors.Length - 1.0f;
					}
				...
				if ((_sizes != null) && (_sizes.Length > 0))
				{
					...
					float max = Mathf.Clamp(Mathf.Ceil(sizeTime), 1.0f, _sizes.Length - 1.0f);
					float lerp = Mathf.InverseLerp(min, max, sizeTime);

					if (min >= _sizes.Length)
					{
						min = _sizes.Length - 1.0f;
					}
				...
				newUV[n * 2] = new Vector2(uvRatio, 0.0f);
				newUV[(n * 2) + 1] = new Vector2(uvRatio, 1.0f);
