...
public class LowRepGameOverScript : MonoBehaviour
{
	...
	void Start()
	{
		...
		Yandere.transform.parent = transform;
	...
	void Update()
	{
		Darkness.material.color = new Color(
			Darkness.material.color.r,
			Darkness.material.color.g,
			Darkness.material.color.b,
			Darkness.material.color.a - (Time.deltaTime * .5f));

		if (Phase == 0)
		{
			if (Yandere.CharacterAnimation["f02_LowRepGO_A"].time >= 3.0f)
			{
				GigglePhase = 1;
			}
		...
		{
			MyCamera.position += MyCamera.forward * Time.deltaTime * -.1f;

			Timer += Time.deltaTime;

			if (Timer > 2 || Input.GetButtonDown("A"))
			{
		...
		else if (Phase == 6)
		{
			...
			if (Yandere.CharacterAnimation["f02_LowRepGO_D"].time >= Yandere.CharacterAnimation["f02_LowRepGO_D"].length || Input.GetButtonDown("A"))
			{
				...
				Senpai.CharacterAnimation[Senpai.AngryFaceAnim].weight = 1.0f;
		...
		else if (Phase == 7)
		{
			...
			if (Timer > 2 || Input.GetButtonDown("A"))
			{
				Senpai.CharacterAnimation["refuse_01"].speed = .5f;
		...
		else if (Phase == 8)
		{
			...
			{
				Yandere.CharacterAnimation.Play(AnimNames.FemaleScaredIdle);
		...
		if (Phase < 7)
		{
			...
			Debug.Log("Should be playing idle anim.");
		}
		else
		{
			if (Timer > 2)
			{
				...
				Debug.Log("Should be playing head-shake anim.");
			}
			else
			{
				Senpai.CharacterAnimation.Play(Senpai.OriginalIdleAnim);
	...
	void Giggle()
	{
		GiggleID = Random.Range(1, Giggles.Length);

		while (GiggleID == PreviousGiggle)
		{
			GiggleID = Random.Range(1, Giggles.Length);
		}
